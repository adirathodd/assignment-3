...
class App extends Component {
  ...
  renderChart() {
    ...
    const xScale = d3.scaleLinear().domain([0, data.length - 1]).range([margin, 1000 - margin]);
    const fontSizeScale = d3.scaleLinear().domain(d3.extent(data, d => d[1])).range([20, 70]);

    svg.selectAll('text')
      .data(data, d => d[0])
      .join(
        // enter phase to create new elements
        ...
        // update phase to resize and reposition words
        update => update
          .transition()
          .duration(1000)
          .attr('x', (d, i) => xScale(i))
        ...
        // exit phase to fade out and remove older
        exit => exit
          .transition()
          .duration(500)
          .style('opacity', 0)
